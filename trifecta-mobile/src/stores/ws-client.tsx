...
class WsRpcClient {
  ...
  private latestServerConfig: ServerConfig | null = null;
  private receivedStreamConfig = false;
      ...
      ws.onopen = () => {
        ...
        if (!this.configUnsubscribe) {
          this.configUnsubscribe = this.subscribe(
            ...
            {},
            (value) => this.handleServerConfigValue(value),
          );
        }
        this.fetchServerConfig();
        // Match the desktop client: an initial snapshot can contain pending
        // probe results, so request a fresh provider scan and consume the
        // resulting providerStatuses event from the config stream.
        void this.request("server.refreshProviders", {}).catch(() => undefined);
        this.startHeartbeat();
      };
  ...
  private async fetchServerConfig() {
    try {
      const raw = await this.request("server.getConfig", {});
      if (this.receivedStreamConfig) return;
      const config = normalizeServerConfig(raw);
      if (config) this.publishServerConfig(config);
    } catch {}
  }

  private handleServerConfigValue(value: unknown) {
    const config = applyServerConfigStreamValue(this.latestServerConfig, value);
    if (!config || config === this.latestServerConfig) return;
    this.receivedStreamConfig = true;
    this.publishServerConfig(config);
  }

  private publishServerConfig(config: ServerConfig) {
    this.latestServerConfig = config;
