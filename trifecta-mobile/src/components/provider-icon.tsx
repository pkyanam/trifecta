...
function GrokIcon({ size, isDark }: { size: number; isDark: boolean }) {
  const fill = isDark ? "#F5F5F5" : "#0F0F0F";
  return (
    <Svg viewBox="0 0 24 24" width={size} height={size}>
      <Path
        // Grok Build logo from trifecta-desktop Icons.tsx GrokIcon
...
export function ProviderIcon({ driver, label, size = 24 }: Props) {
  ...
  switch (driverToKind(driver)) {
    ...
    case "grok":
      return <GrokIcon size={size} isDark={isDark} />;
    case "gemini":
      return <GeminiIcon size={size} />;
    case "copilot":
