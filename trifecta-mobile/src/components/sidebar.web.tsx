...
export function Sidebar({
  ...
  const { activeThreadId, setActiveThreadId, startNewChat } = useActiveThread();

  const threadsByProject = useMemo(() => {
    const map = new Map<string, typeof activeThreads>();
  ...
  const activeProjects = projects.filter(
    (p) => (threadsByProject.get(p.id)?.length ?? 0) > 0,
  );
        ...
        {!isCollapsed && (
          ...
          >
            ...
            {activeProjects.map((project) => {
              ...
              return (
                <View key={project.id}>
                  <Text className="text-[11px] font-semibold text-muted-foreground/60 px-6 pt-5 pb-1.5 uppercase tracking-wider">
                        ...
                        <ContextMenu.Trigger asChild>
                          <Link href="/" asChild>
                            ...
                            >
                              <Text
                                numberOfLines={1}
                                className={`text-[15px] ${isActive ? "text-foreground" : "text-muted-foreground"}`}
                              >
        ...
        {isCollapsed && (
          <Tooltip.Provider delayDuration={200}>
            <View className="flex flex-col items-center gap-1 pt-3 px-1.5">
              ...
              <SidebarTooltip label="New thread">
                <Link href="/" asChild>
                  <Pressable
              ...
              <SidebarTooltip label="Delete thread">
          ...
          <View className="border-t border-border/40 px-3 py-3">
            <View className="flex flex-row items-center">
              ...
              <Link href="/" asChild>
                <Pressable
                  onPress={() => startNewChat()}
                  className="w-10 h-10 rounded-full bg-foreground hover:bg-foreground/90 active:bg-foreground/80 items-center justify-center flex"
                >
