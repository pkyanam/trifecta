...
struct TurnTimelineView<EmptyState: View, Composer: View>: View {
    ...
    @State private var cachedPlanMatchingFingerprint: Int = 0
    ...
    private var planMatchingFingerprint: Int {
        cachedPlanMatchingFingerprint
    }

    private func recomputePlanMatchingFingerprint() {
        var hasher = Hasher()
        ...
        cachedPlanMatchingFingerprint = hasher.finalize()
    }
    ...
    private func timelineScrollView(proxy: ScrollViewProxy, viewport: GeometryProxy) -> some View {
        timelineScrollObservers2(proxy: proxy, viewportWidth: viewport.size.width)
            .safeAreaInset(edge: .bottom, spacing: 0) {
                footer(scrollToBottomAction: { handleScrollToLatestButtonTap(using: proxy) })
            }
            .onAppear {
    ...
    @ViewBuilder
    private func timelineScrollObservers2(proxy: ScrollViewProxy, viewportWidth: CGFloat) -> some View {
        timelineScrollObservers1(proxy: proxy, viewportWidth: viewportWidth)
            .onChange(of: threadID) { _, _ in
    ...
    @ViewBuilder
    private func timelineScrollObservers1(proxy: ScrollViewProxy, viewportWidth: CGFloat) -> some View {
        let contentWidth = timelineContentWidth(for: viewportWidth)
        timelineScrollBase(proxy: proxy, viewportWidth: viewportWidth, contentWidth: contentWidth)
            ...
            .onChange(of: hasRemoteEarlierMessages) { _, newValue in
                if !newValue { pendingRemoteEarlierLoadMessageCount = nil }
            }
            .onChange(of: olderHistoryLoadErrorMessage) { _, newValue in
                if newValue != nil { pendingRemoteEarlierLoadMessageCount = nil }
            }
    ...
    @ViewBuilder
    private func timelineScrollBase(proxy: ScrollViewProxy, viewportWidth: CGFloat, contentWidth: CGFloat) -> some View {
        ScrollView(.vertical) {
        ...
        .onScrollGeometryChange(for: ScrollBottomGeometry.self) { geometry in
            ...
            scrollGeometryCoalescer.applyTask = Task { @MainActor in
                ...
                guard !isEarlierHistoryInteractionActive else { return }
                applyScrollGeometryUpdate(old: pending.old, new: pending.new, using: proxy)
            }
        }
    }
    ...
    private func recomputeRenderItemsIfNeeded() {
        recomputePlanMatchingFingerprint()
        let signature = renderItemsCacheSignature(for: visibleMessages)
