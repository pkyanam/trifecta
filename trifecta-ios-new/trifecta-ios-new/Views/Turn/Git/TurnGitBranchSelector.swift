...
func trifectaNormalizedCreatedBranchName(_ rawName: String) -> String {
    let trimmedName = rawName.trimmingCharacters(in: .whitespacesAndNewlines)
    guard !trimmedName.isEmpty else { return "" }
    if trimmedName.hasPrefix("trifecta/") {
        return trimmedName
    }
    return "trifecta/\(trimmedName)"
}

// Leaves "open elsewhere" branches selectable so the caller can surface the right alert or git error.
func trifectaCurrentBranchSelectionIsDisabled(
    branch: String,
...
func trifectaSelectableDefaultBranch(
...
struct TurnGitBranchSelector: View, Equatable {
    ...
    private var localDefaultBranch: String? {
        ...
        return trifectaSelectableDefaultBranch(
...
struct TurnGitBranchPickerSheet: View {
    ...
    private var isNewBranchNameValid: Bool {
        ...
        guard !trimmed.isEmpty, trimmed != "trifecta/" else { return false }
        return true
    }
    ...
    private var suggestedCreateBranchName: String? {
        ...
        let candidate = trifectaNormalizedCreatedBranchName(searchText)
    ...
    var body: some View {
        List {
            Section(sectionTitle) {
                if showsDefaultBranchRow, let defaultBranch {
                    let isDisabled = trifectaCurrentBranchSelectionIsDisabled(
            ...
            if allowsSelectingCurrentBranch {
                Section {
                    ...
                    Button {
                        let fromSearch = trifectaNormalizedCreatedBranchName(searchText)
                        newBranchName = fromSearch.isEmpty ? "trifecta/" : fromSearch
        ...
        .alert("New branch", isPresented: $isShowingCreateBranchPrompt) {
            TextField("trifecta/my-feature", text: $newBranchName)
            Button("Cancel", role: .cancel) {
                newBranchName = ""
            }
            Button("Create") {
                let branchName = trifectaNormalizedCreatedBranchName(newBranchName)
...
#Preview("Branch Picker") {
    TurnGitBranchPickerSheet(
        branches: [
            "feature/auth-flow",
            "feature/dark-mode",
            "trifecta/onboarding-v2",
            "trifecta/sidebar-refactor",
            "fix/crash-on-launch",
        ...
        selectedBranch: "trifecta/onboarding-v2",
        defaultBranch: "main",
        currentBranch: "trifecta/onboarding-v2",
