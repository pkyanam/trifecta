...
struct TurnComposerHostView: View {
    ...
    var body: some View {
        ...
        let effectiveModel = codex.selectedModelOption(for: thread.id)
        let selectedModelID = effectiveModel?.id
            ?? codex.effectiveComposerModelSelection(for: thread.id).model
        ...
        TurnComposerView(
            ...
            selectedAccessMode: codex.effectiveComposerRuntimeMode(for: thread.id),
            ...
            onSelectAccessMode: { mode in
                codex.setComposerRuntimeMode(mode, for: thread.id)
            },
