...
struct TurnView: View {
    ...
    private var timelineFooterErrorMessage: String? {
        TurnFooterErrorFilter.visibleFooterMessage(from: codex.visibleErrorMessage)
    }
    ...
    private var connectionRecoverySnapshot: ConnectionRecoverySnapshot? {
        TurnConnectionRecoverySnapshotBuilder.makeSnapshot(
            ...
            lastErrorMessage: codex.visibleErrorMessage
        )
    }
    ...
    private var reasoningDisplayOptions: [TurnComposerReasoningDisplayOption] {
        TurnComposerMetaMapper.reasoningDisplayOptions(
            from: codex.supportedReasoningEfforts(for: thread.id).map(\.reasoningEffort)
        )
    }

    private var selectedModelTitle: String {
        if let selectedModel = codex.selectedModelOption(for: thread.id) {
            return TurnComposerMetaMapper.modelTitle(for: selectedModel)
        }

        let selection = codex.effectiveComposerModelSelection(for: thread.id)
        if let model = selection.model {
            return TurnComposerMetaMapper.modelTitle(forIdentifier: model)
        }
