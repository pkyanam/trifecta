...
final class TurnViewModel {
    ...
    // Debounces installed Trifecta plugin suggestions for `@plugin` composer mentions.
    ...
    static func removeBoundedToken(
        ...
        let cacheKey = "remove:" + escaped + ":" + (caseInsensitive ? "1" : "0")
        let regex = TurnViewModel.boundedTokenRegexCache(for: cacheKey, pattern: escaped + "(?:[\\s,.;:!?)\\]}>]|$)", caseInsensitive: caseInsensitive)
    ...
    static func replaceBoundedToken(
        ...
        let escaped = NSRegularExpression.escapedPattern(for: token)
        let cacheKey = "replace:" + escaped + ":" + (caseInsensitive ? "1" : "0")
        let regex = TurnViewModel.boundedTokenRegexCache(for: cacheKey, pattern: escaped + "(?=[\\s,.;:!?)\\]}>]|$)", caseInsensitive: caseInsensitive)
        let range = NSRange(text.startIndex..., in: text)
    ...
    private static var _boundedTokenRegexCache: [String: NSRegularExpression] = [:]
    private static let _boundedTokenRegexCacheLock = NSLock()

    private static func boundedTokenRegexCache(for key: String, pattern: String, caseInsensitive: Bool) -> NSRegularExpression {
        _boundedTokenRegexCacheLock.lock()
        defer { _boundedTokenRegexCacheLock.unlock() }
        if let cached = _boundedTokenRegexCache[key] {
            return cached
        }
        let options: NSRegularExpression.Options = caseInsensitive ? [.caseInsensitive] : []
        let regex = (try? NSRegularExpression(pattern: pattern, options: options)) ?? NSRegularExpression()
        _boundedTokenRegexCache[key] = regex
        return regex
    }
