...
struct BridgeUpdateSheet: View {
    ...
    private var isComputerUpdatePrompt: Bool {
        let text = "\(prompt.title) \(prompt.message)".localizedLowercase
        return text.contains("computer") || text.contains("bridge") || text.contains("npm") || text.contains("cli")
    }

    private var fallbackInstruction: String {
        if isComputerUpdatePrompt {
            return "Update Trifecta Desktop or the Trifecta CLI on your computer, restart the bridge, then reconnect from this iPhone."
        }
        return "Install the latest Trifecta build on this iPhone, then come back here and reconnect."
    }

    private var footerInstruction: String {
        if prompt.command != nil || isComputerUpdatePrompt {
            return "After the update finishes, restart the bridge on your computer and come back here."
        }
        return "After the app finishes updating on your iPhone, reconnect to the computer bridge."
    }
}
