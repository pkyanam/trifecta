...
final class ContentViewModel {
    ...
    func connectToRelay(pairingPayload: CodexPairingQRPayload, codex: CodexService) async {
        ...
        if isTrifectaPairingPayload(pairingPayload) {
            do {
                try await codex.connectToTrifectaDesktop(pairingPayload: pairingPayload)
            } catch {
...
extension ContentViewModel {
    ...
    func connect(codex: CodexService, serverURL: String) async throws {
        ...
        // Reconstruct the Trifecta Desktop bridge from saved pairing rather than treating
        // the relay URL as a WebSocket relay endpoint.
        if let macDeviceId = codex.normalizedRelayMacDeviceId,
           let sessionId = codex.normalizedRelaySessionId,
           let relayURL = codex.normalizedRelayURL,
           let identityKey = codex.normalizedRelayMacIdentityPublicKey {
            let saved = makeTrifectaPairingPayload(serverURL: relayURL, token: sessionId)
            let corrected = CodexPairingQRPayload(
                v: saved.v,
                relay: relayURL,
                sessionId: sessionId,
                macDeviceId: macDeviceId,
                macIdentityPublicKey: identityKey,
                expiresAt: saved.expiresAt
            )
            if isTrifectaPairingPayload(corrected) {
                try await codex.connectToTrifectaDesktop(pairingPayload: corrected)
                return
            }
        }
