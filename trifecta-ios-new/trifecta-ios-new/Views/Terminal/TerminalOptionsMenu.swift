...
struct TerminalOptionsMenu: View {
    ...
    let theme: TrifectaTerminalTheme
    let fontSize: Double
    ...
    private var textSizeSection: some View {
        Section("Text size") {
            Button("A- \(String(format: "%.1f", nextSmallerFontSize)) pt") {
                onAdjustFontSize(-trifectaTerminalFontSizeStep)
            }
            .disabled(fontSize <= trifectaTerminalMinFontSize)

            Button("A+ \(String(format: "%.1f", nextLargerFontSize)) pt") {
                onAdjustFontSize(trifectaTerminalFontSizeStep)
            }
            .disabled(fontSize >= trifectaTerminalMaxFontSize)
        }
    }
    ...
    private var nextSmallerFontSize: Double {
        max(trifectaTerminalMinFontSize, fontSize - trifectaTerminalFontSizeStep)
    }

    private var nextLargerFontSize: Double {
        min(trifectaTerminalMaxFontSize, fontSize + trifectaTerminalFontSizeStep)
    }
}
