...
// Depends on: CodexService, GhosttyTerminalSurface, TrifectaTerminalModels
...
struct TerminalScreen: View {
    ...
    @State private var draftProfile = TrifectaTerminalProfileStore.load()
    @State private var connectionDraft = TrifectaTerminalProfileStore.load().connectionString
    @State private var privateKeyDraft = TrifectaTerminalPrivateKeyStore.loadPrivateKey()
    @State private var passphraseDraft = TrifectaTerminalPrivateKeyStore.loadPassphrase()
    ...
    private var profileResolvedFromConnection: TrifectaTerminalProfile {
    ...
    private var hasConnectionConfiguration: Bool {
        ...
        return !profile.host.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty
            ...
            && TrifectaTerminalPrivateKeyStore.hasPrivateKey(privateKeyDraft)
    }
    ...
    private var activeSnapshot: TrifectaTerminalSnapshot {
    ...
    private func openTerminal() async {
        ...
        actionErrorMessage = nil
        TrifectaTerminalProfileStore.save(draftProfile)
        TrifectaTerminalPrivateKeyStore.savePrivateKey(privateKeyDraft)
        TrifectaTerminalPrivateKeyStore.savePassphrase(passphraseDraft)
    ...
    private func resetKnownHost() {
        ...
        TrifectaSSHKnownHostStore.delete(host: profile.host, port: profile.port)
        actionErrorMessage = nil
    }
    ...
    private func adjustFontSize(_ delta: Double) {
        terminalFontSize = min(
            trifectaTerminalMaxFontSize,
            max(trifectaTerminalMinFontSize, terminalFontSize + delta)
        )
    }
