...
struct SidebarThreadListView: View {
    ...
    let loadSubagentMetadata: ([String]) async -> Void
    ...
    var body: some View {
        ...
        .task(id: visibleSubagentThreadIDs) {
            await loadSubagentMetadata(visibleSubagentThreadIDs)
        }
        .onAppear {
    ...
    private func threadRow(
        ...
        return SidebarThreadRowView(
            ...
            isPinned: Self.threadIsPinned(thread, pinnedThreadIDs: pinnedThreadIDs),
    ...
    private static func threadIsPinned(_ thread: CodexThread, pinnedThreadIDs: Set<String>) -> Bool {
        if pinnedThreadIDs.contains(thread.id) { return true }
        if let forkParentID = thread.forkedFromThreadId {
            if pinnedThreadIDs.contains(forkParentID) { return true }
        }
        return false
    }
