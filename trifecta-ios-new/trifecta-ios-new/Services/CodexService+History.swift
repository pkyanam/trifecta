...
extension CodexService {
    ...
    private func decodeSubagentReceiverThreadIDs(from itemObject: [String: JSONValue]) -> [String] {
        ...
        // Fallback: singular top-level field (Trifecta CLI sends one event per agent).
    ...
    private func decodeSubagentReceiverAgents(
        ...
        if candidate?.arrayValue == nil || candidate?.arrayValue?.isEmpty == true {
            // Trifecta CLI sends singular top-level identity fields per event.
    ...
    // Builds a single-element agent ref array from top-level fields when the Trifecta CLI sends
