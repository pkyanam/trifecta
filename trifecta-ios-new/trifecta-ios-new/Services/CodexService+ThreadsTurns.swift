...
extension CodexService {
    ...
    // Loads Trifecta app-server plugins and returns entries usable as `@plugin` mentions.
    func listPlugins(
        cwds: [String]?,
        forceReload: Bool = false
...
extension CodexService {
    ...
    func buildTurnStartRequestParams(
        ...
        let modelSelectionPayload = composerModelSelectionPayload(for: threadId)
        if !modelSelectionPayload.isEmpty {
            params["modelSelection"] = .object(
                modelSelectionPayload.mapValues { JSONValue(jsonObject: $0) }
            )
        }
        params["runtimeMode"] = .string(composerRuntimeModePayload(for: threadId))
    ...
    // Parses Trifecta app-server plugin/list marketplace payloads.
