...
extension CodexService {
    ...
    func upsertThread(_ incomingThread: CodexThread, treatAsServerState: Bool = false) {
        ...
        if activeThreadId == resolvedThread.id,
           treatAsServerState || !(composerDraftsByThreadID[resolvedThread.id]?.hasLocalEdits ?? false) {
            hydrateComposerDraft(for: resolvedThread.id)
        }
