...
extension CodexService {
    ...
    // Refreshes only the bridge package version state so Trifecta updates stay independent from GPT UX.
...
extension CodexService {
    ...
    private func minimumBridgePackageUpdatePrompt(currentVersion: String?) -> CodexBridgeUpdatePrompt {
        ...
        if let currentVersion = currentVersion?.trimmingCharacters(in: .whitespacesAndNewlines),
           !currentVersion.isEmpty {
            message =
                "This computer bridge is running Trifecta \(currentVersion), but this iPhone app requires Trifecta \(CodexService.minimumSupportedBridgePackageVersion) or newer. Update the npm package on your computer, then reconnect."
        } else {
            message =
                "This computer bridge is too old for this version of Trifecta iPhone. Update the Trifecta npm package on your computer to \(CodexService.minimumSupportedBridgePackageVersion) or newer, then reconnect."
        }

        return CodexBridgeUpdatePrompt(
            title: "Update Trifecta on your computer to reconnect",
            message: message,
            command: nil
        )
    }
    ...
    private func availableBridgePackageUpdatePrompt(
        ...
        CodexBridgeUpdatePrompt(
            title: "A newer Trifecta update is available on your computer",
            message: "This computer bridge is running Trifecta \(currentVersion), and npm now has Trifecta \(latestVersion). Update the package on your computer when you're ready, then reconnect to start using the newer build.",
            command: nil
        )
    }
    ...
    private func forcedBridgePackageUpdatePrompt(currentVersion: String) -> CodexBridgeUpdatePrompt {
        CodexBridgeUpdatePrompt(
            title: "Update Trifecta on your computer to reconnect",
            message: "This computer bridge is running Trifecta \(currentVersion). Update the Trifecta CLI on your computer to \(forcedBridgeUpgradeTargetVersion), then reconnect.",
            command: nil
        )
    }
