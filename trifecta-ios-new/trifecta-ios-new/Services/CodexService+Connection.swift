...
extension CodexService {
    ...
    func initializeSession() async throws {
        ...
        let clientInfo: JSONValue = .object([
            "name": .string("trifecta_ios"),
            "title": .string("Trifecta iOS"),
    ...
    func incompatibleBridgeAppVersionError(from error: Error) -> CodexServiceError? {
        ...
        if !message.isEmpty {
            ...
            promptMessage =
                "This computer bridge is running Trifecta \(bridgeVersion), which requires Trifecta iPhone \(minimumSupportedAppVersion) or newer. Update the iPhone app, then reconnect."
        } else if let minimumSupportedAppVersion, !minimumSupportedAppVersion.isEmpty {
            promptMessage =
                "This computer bridge requires Trifecta iPhone \(minimumSupportedAppVersion) or newer. Update the iPhone app, then reconnect."
        } else {
            promptMessage = "This computer bridge requires a newer Trifecta iPhone app. Update the app, then reconnect."
        }

        bridgeUpdatePrompt = CodexBridgeUpdatePrompt(
            title: "Update Trifecta on your iPhone to reconnect",
        ...
        return .invalidInput("This computer bridge requires a newer Trifecta iPhone app. Update the app, then reconnect.")
    }
...
extension CodexService {
    ...
    func userFacingConnectError(error: Error, attemptedURL: String, host: String?) -> String {
        ...
        if nsError.domain == NSURLErrorDomain,
           ...
           requiresLocalNetworkAuthorization(for: URL(string: attemptedURL) ?? URL(fileURLWithPath: "/")) {
            return "Trifecta cannot open the local relay connection on this iPhone. Check Local Network and the app's Wi-Fi/Cellular access in Settings, then retry."
        }
    ...
    func retryableSessionUnavailableMessage(for closeCode: NWProtocolWebSocket.CloseCode?) -> String? {
        ...
        return "Trying to reach your saved computer. Trifecta will keep retrying. If you restarted the bridge on that computer, scan the new QR code."
    }
    ...
    func requestLocalNetworkAuthorizationIfNeeded(for url: URL) async throws {
        ...
        guard status != .denied else {
            let message =
                "Trifecta is not allowed to access your local network. Enable Local Network for Trifecta in iPhone Settings and try again."
            lastErrorMessage = message
