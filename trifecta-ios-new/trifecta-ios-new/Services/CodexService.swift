...
// Purpose: Central state container for Trifecta app-server communication.
// Layer: Service
...
final class CodexService {
    ...
    var activeThreadId: String? {
        didSet {
            if let newID = activeThreadId, newID != oldValue {
                touchThreadMessages(newID)
                hydrateComposerDraft(for: newID)
            }
            if oldValue != activeThreadId {
                evictStaleThreadMessages()
            }
        }
    }
    var activeTurnId: String?
    ...
    var lastErrorMessage: String? {
        didSet {
            // Coalesce rapid mutations: only trigger observation when the value actually changes.
            // Setting nil when already nil, or the same message, is a no-op for observation.
            if lastErrorMessage == oldValue { return }
            lastErrorMessageDebounceTask?.cancel()
            if lastErrorMessage != nil {
                // Post error messages immediately so the user sees them.
                visibleErrorMessage = lastErrorMessage
            } else {
                // Delay clearing the error slightly to batch with other state changes
                // (e.g., connectionRecoveryState updates in the same reconnect cycle).
                lastErrorMessageDebounceTask = Task { @MainActor [weak self] in
                    try? await Task.sleep(nanoseconds: 50_000_000) // 50ms
                    guard !Task.isCancelled else { return }
                    self?.visibleErrorMessage = nil
                }
            }
        }
    }
    // The publicly visible error message, debounced to avoid render storms.
    var visibleErrorMessage: String?
    var lastErrorMessageDebounceTask: Task<Void, Never>?
    ...
    // Tracks when each thread's messages were last accessed, used for LRU eviction.
    @ObservationIgnored var messagesByThreadLastAccessedAt: [String: Date] = [:]
    // Maximum number of threads whose messages are kept in memory simultaneously.
    // The active thread is always kept regardless of this limit.
    static let maxInMemoryThreadMessageBatches = 8
    var syncRealtimeEnabled = true
    var availableModels: [CodexModelOption] = []
    @ObservationIgnored var availableModelsById: [String: CodexModelOption] = [:]
    @ObservationIgnored var availableModelsByModel: [String: CodexModelOption] = [:]
    @ObservationIgnored var availableModelsByProviderAndModel: [String: CodexModelOption] = [:]
    ...
    var composerDraftsByThreadID: [String: ComposerThreadDraft] = [:]
    var selectedAccessMode: CodexAccessMode = .approvalRequired
    ...
    var terminalSnapshot: TrifectaTerminalSnapshot = .idle
    var terminalSnapshotsById: [String: TrifectaTerminalSnapshot] = [:]
    var terminalProfile: TrifectaTerminalProfile = TrifectaTerminalProfileStore.load()
    @ObservationIgnored let nativeSSHTerminal = TrifectaNativeSSHTerminal()
    @ObservationIgnored var nativeSSHTerminalsById: [String: TrifectaNativeSSHTerminal] = [:]
    ...
    let webSocketQueue = DispatchQueue(label: "TrifectaIOS.WebSocket", qos: .userInitiated)
    ...
    @ObservationIgnored var trifectaDesktopBridge: TrifectaDesktopBridge?
    ...
    // Tracks whether the bridge is proxying a real Trifecta endpoint or a spawned local app-server.
    ...
    init(
        ...
        if let savedAccessMode = defaults.string(forKey: Self.selectedAccessModeDefaultsKey) {
            self.selectedAccessMode = CodexAccessMode(canonicalRawValue: savedAccessMode) ?? .approvalRequired
        } else {
            self.selectedAccessMode = .approvalRequired
        }

        loadComposerDraftsFromDefaults()

        // GPT account snapshot loaded in loadPersistedState() for startup performance
    ...
    // Until this is true, messagesByThread is empty and views should show a loading state.
    var hasLoadedPersistedState = false

    // Loads persisted message history and change sets in the background.
    // Call this after init to avoid blocking the main thread during app launch.
    // Until this completes, messagesByThread is empty and hasLoadedPersistedState is false.
    func loadPersistedState() {
        guard !hasLoadedPersistedState else { return }

        let messagePersistence = self.messagePersistence
        let aiChangeSetPersistence = self.aiChangeSetPersistence

        Task { @MainActor in
            let loadedMessages = await Task.detached {
                messagePersistence.load().mapValues { messages in
            ...
            let loadedChangeSets = await Task.detached {
            ...
            self.hasLoadedPersistedState = true
        }
    }
