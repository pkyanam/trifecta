...
extension CodexService {
    ...
    func listModels() async throws {
        ...
        do {
            ...
            rebuildAvailableModelLookupCaches()
    ...
    func selectedReasoningEffortForSelectedModel(threadId: String? = nil) -> String? {
        ...
        if let normalizedThreadID = normalizedInterruptIdentifier(threadId) {
            modelOption = selectedModelOption(for: normalizedThreadID)
        } else {
            modelOption = selectedModelOption()
        }

        guard let model = modelOption else {
    ...
    func runtimeModelIdentifierForTurn(threadId: String? = nil) -> String? {
        ...
        if let resolvedThreadId {
            let selection = effectiveComposerModelSelection(for: resolvedThreadId)
            if let model = selection.model?.trimmingCharacters(in: .whitespacesAndNewlines),
               !model.isEmpty {
                return model
            }
            if let model = selectedModelOption(for: resolvedThreadId)?.model {
                return model
            }
        }

        return selectedModelOption()?.model
            ?? selectedModelId
...
private extension CodexService {
    func rebuildAvailableModelLookupCaches() {
        availableModelsById.removeAll(keepingCapacity: true)
        availableModelsByModel.removeAll(keepingCapacity: true)
        availableModelsByProviderAndModel.removeAll(keepingCapacity: true)

        for model in availableModels {
            availableModelsById[model.id] = model
            if availableModelsByModel[model.model] == nil {
                availableModelsByModel[model.model] = model
            }
            if let providerId = model.providerId {
                availableModelsByProviderAndModel[modelLookupKey(providerId: providerId, model: model.model)] = model
            }
        }
    }

    func modelLookupKey(providerId: String, model: String) -> String {
        "\(providerId.trimmingCharacters(in: .whitespacesAndNewlines))|\(model.trimmingCharacters(in: .whitespacesAndNewlines))"
    }
    ...
    func normalizeRuntimeSelectionsAfterModelsUpdate() {
        guard !availableModels.isEmpty else {
            availableModelsById.removeAll()
            availableModelsByModel.removeAll()
            availableModelsByProviderAndModel.removeAll()
    ...
    func selectedModelOption(from models: [CodexModelOption]) -> CodexModelOption? {
        ...
        if models.count == availableModels.count,
           let selectedModelId {
            if let byId = availableModelsById[selectedModelId] {
                return byId
            }
            if let byModel = availableModelsByModel[selectedModelId] {
                return byModel
            }
    ...
    func selectedGitWriterModelOption(
        ...
        if models.count == availableModels.count,
           let savedSelection {
            if let byId = availableModelsById[savedSelection] {
                return byId
            }
            if let byModel = availableModelsByModel[savedSelection] {
                return byModel
            }
