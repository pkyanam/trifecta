// FILE: TrifectaTerminalModels.swift
// Purpose: Defines the native SSH terminal profile and runtime snapshot models.
// Layer: Service Model
// Exports: TrifectaTerminalProfile, TrifectaTerminalSnapshot, TrifectaTerminalStatus
// Depends on: Foundation, JSONValue

import Foundation

private let trifectaTerminalMaxBufferCharacters = 200_000
private let trifectaTerminalMaxBufferBytes = 200_000
private let trifectaDefaultTerminalId = "term-1"

enum TrifectaTerminalStatus: String, Codable, Equatable, Sendable {
    case idle
    case starting
    case running
    case exited
...
struct TrifectaTerminalProfile: Codable, Equatable, Sendable {
    var host: String
    var username: String
    var port: Int
    var cwd: String
    ...
    static var empty: TrifectaTerminalProfile {
        TrifectaTerminalProfile(
            host: "",
            username: "",
            port: 22,
            cwd: "",
    ...
    var normalizedForSave: TrifectaTerminalProfile {
        TrifectaTerminalProfile(
...
struct TrifectaTerminalSnapshot: Equatable, Sendable {
    var terminalId: String
    var instanceId: String?
    var status: TrifectaTerminalStatus
    var buffer: String
    var bufferData: Data
    var cwd: String
    var cols: Int
    ...
    static let idle = TrifectaTerminalSnapshot(
        terminalId: trifectaDefaultTerminalId,
        instanceId: nil,
        status: .idle,
        buffer: "",
        bufferData: Data(),
    ...
    static func idleSnapshot(terminalId: String) -> TrifectaTerminalSnapshot {
        var snapshot = idle
    ...
    init(resultObject: [String: JSONValue]) {
        let rawStatus = resultObject["status"]?.stringValue ?? TrifectaTerminalStatus.idle.rawValue
        ...
        self.init(
            terminalId: resultObject["terminalId"]?.stringValue ?? resultObject["terminal_id"]?.stringValue ?? trifectaDefaultTerminalId,
            instanceId: Self.instanceId(in: resultObject),
            status: TrifectaTerminalStatus(rawValue: rawStatus) ?? .idle,
    ...
    mutating func applyTerminalEvent(_ paramsObject: [String: JSONValue]) {
        ...
        if let rawStatus = paramsObject["status"]?.stringValue,
           let status = TrifectaTerminalStatus(rawValue: rawStatus) {
            self.status = status
        }
    ...
    private static func trimmedBuffer(_ value: String) -> String {
        guard value.count > trifectaTerminalMaxBufferCharacters else {
            return value
        }
        return String(value.suffix(trifectaTerminalMaxBufferCharacters))
    }

    private static func trimmedBufferData(_ value: Data) -> Data {
        guard value.count > trifectaTerminalMaxBufferBytes else {
            return value
        }
        return Data(value.suffix(trifectaTerminalMaxBufferBytes))
    }
