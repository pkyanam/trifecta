...
// Depends on: CodexService, TrifectaNativeSSHTerminal, TrifectaTerminalModels
...
extension CodexService {
    ...
    func terminalSnapshot(for terminalId: String) -> TrifectaTerminalSnapshot {
        if terminalId == Self.defaultTerminalId, terminalSnapshotsById[terminalId] == nil {
            return terminalSnapshot
        }
        return terminalSnapshotsById[terminalId] ?? TrifectaTerminalSnapshot.idleSnapshot(terminalId: terminalId)
    }

    func knownTerminalSnapshots() -> [TrifectaTerminalSnapshot] {
    ...
    func openTerminal(
        profile: TrifectaTerminalProfile,
        cols: Int,
        rows: Int
    ) async throws {
    ...
    func openTerminal(
        ...
        TrifectaTerminalProfileStore.save(normalizedProfile)
        setTerminalSnapshot(TrifectaTerminalSnapshot(
        ...
        let privateKey = TrifectaTerminalPrivateKeyStore.loadPrivateKey()
        let passphrase = TrifectaTerminalPrivateKeyStore.loadPassphrase()
        do {
    ...
    func changeTerminalWorkingDirectory(_ cwd: String, terminalId: String) async throws {
        ...
        TrifectaTerminalProfileStore.save(terminalProfile)
        guard terminalSnapshot(for: terminalId).status == .running else { return }
    ...
    private func nativeTerminal(for terminalId: String) -> TrifectaNativeSSHTerminal {
        ...
        let terminal = TrifectaNativeSSHTerminal()
        nativeSSHTerminalsById[terminalId] = terminal
        return terminal
    }

    private func setTerminalSnapshot(_ snapshot: TrifectaTerminalSnapshot, for terminalId: String) {
    ...
    private func updateTerminalSnapshot(
        ...
        mutate: (inout TrifectaTerminalSnapshot) -> Void
    ) {
