// FILE: TrifectaNativeSSHTerminal.swift
// Purpose: Owns the phone-side SSH client and bridges raw TTY bytes to Ghostty.
// Layer: Service
// Exports: TrifectaNativeSSHTerminal, TrifectaNativeSSHTerminalError
...
enum TrifectaNativeSSHTerminalError: LocalizedError {
...
@MainActor
final class TrifectaNativeSSHTerminal {
            ...
            sessionTask = Task { [weak self] in
                ...
                do {
                    let sshClient = try await SSHClient.connect(
                        ...
                        hostKeyValidator: .custom(TrifectaSSHKnownHostValidator(
                            host: profile.host,
                            port: profile.port
                        )),
                        reconnect: .never
    ...
    func write(_ data: Data) async throws {
        guard let writer else {
            throw TrifectaNativeSSHTerminalError.sessionNotRunning
        }
    ...
    private static func authenticationMethod(
        ...
        guard !normalizedKey.isEmpty else {
            throw TrifectaNativeSSHTerminalError.missingPrivateKey
        }
        ...
        default:
            throw TrifectaNativeSSHTerminalError.unsupportedPrivateKey(keyType.description)
        }
    }
}

private struct TrifectaSSHKnownHostValidator: NIOSSHClientServerAuthenticationDelegate, Sendable {
    let host: String
    let port: Int
    ...
    nonisolated func validateHostKey(
        ...
        if let storedHostKey = TrifectaSSHKnownHostStore.load(host: host, port: port) {
            if storedHostKey == currentHostKey {
                validationCompletePromise.succeed(())
            } else {
                validationCompletePromise.fail(TrifectaNativeSSHTerminalError.hostKeyChanged)
            }
            return
        }

        TrifectaSSHKnownHostStore.save(currentHostKey, host: host, port: port)
        validationCompletePromise.succeed(())
    }
}
