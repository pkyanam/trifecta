...
extension CodexService {
    ...
    func startSyncLoop() {
        ...
        // feel closer to live on iPhone even when Trifecta.app itself doesn't push updates.
    ...
    private func removeThreadLocally(_ threadId: String, persistAsDeleted: Bool, persistMessages: Bool = true) {
        ...
        // Prune per-turn terminal state for turns belonging to this thread.
        terminalStateByTurnID = terminalStateByTurnID.filter { key, _ in
            guard let turnThreadId = threadIdByTurnID[key] else {
                // Orphaned turn with no thread mapping — remove it.
                return false
            }
            return turnThreadId != threadId
        }
