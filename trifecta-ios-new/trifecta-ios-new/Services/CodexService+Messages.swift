...
extension CodexService {
    ...
    func messages(for threadId: String) -> [CodexMessage] {
        messagesByThreadLastAccessedAt[threadId] = Date()
    ...
    // The first turn can be running before Trifecta has persisted a readable
    ...
    func markTurnCompleted(threadId: String, turnId: String?) {
        ...
        if var threadMessages = messagesByThread[threadId] {
            ...
            for index in threadMessages.indices where threadMessages[index].role == .assistant
                && threadMessages[index].isStreaming {
                let belongsToTurn = belongsToCompletedTurn(threadMessages[index])
                guard belongsToTurn else { continue }
                threadMessages[index].isStreaming = false
...
extension CodexService {
    ...
    // Evicts in-memory messages for threads not recently accessed, keeping at most
    // `maxInMemoryThreadMessageBatches` threads. The currently active thread is always retained.
    // Evicted thread messages are persisted to disk first so they can be reloaded on demand.
    func evictStaleThreadMessages() {
        let activeID = activeThreadId
        let now = Date()
        let maxThreads = Self.maxInMemoryThreadMessageBatches

        guard messagesByThread.count > maxThreads else { return }

        // Sort threads by last access time, most recent first.
        // Threads with no access timestamp are treated as least recently used.
        let sortedThreadIDs = messagesByThread.keys.sorted { a, b in
            let timeA = messagesByThreadLastAccessedAt[a] ?? .distantPast
            let timeB = messagesByThreadLastAccessedAt[b] ?? .distantPast
            return timeA > timeB
        }

        // Keep the active thread + the most recently accessed threads up to the limit.
        var retainedIDs = Set<String>()
        retainedIDs.insert(activeID ?? "")
        for id in sortedThreadIDs {
            if retainedIDs.count >= maxThreads { break }
            retainedIDs.insert(id)
        }

        // Persist and evict threads outside the retention window.
        // Threads not accessed in the last 30 minutes are also evicted regardless of the limit.
        let staleCutoff = now.addingTimeInterval(-1800)
        var didModify = false
        for id in sortedThreadIDs where !retainedIDs.contains(id) {
            let lastAccess = messagesByThreadLastAccessedAt[id] ?? .distantPast
            if lastAccess < staleCutoff || messagesByThread.count - retainedIDs.count > 0 {
                messagesByThread.removeValue(forKey: id)
                messagesByThreadLastAccessedAt.removeValue(forKey: id)
                messageRevisionByThread.removeValue(forKey: id)
                removeThreadTimelineState(for: id)
                didModify = true
            }
        }
    ...
    // Marks a thread's messages as recently accessed so eviction does not remove them.
    func touchThreadMessages(_ threadId: String) {
        messagesByThreadLastAccessedAt[threadId] = Date()
    }
}
