...
struct ContentView: View {
    ...
    @Environment(PetCompanionStore.self) private var petCompanionStore
    ...
    @State private var hasDismissedPairingSetup = false
    ...
    private var rootContentWithLifecycleObservers: some View {
        rootContent
            ...
            .task {
                guard !isShowingPairingSetup else {
                    debugSidebarLog("launch task skipped pairingSetup=\(isShowingPairingSetup)")
                    return
                }
            ...
            .onChange(of: codex.bridgeUpdatePrompt) { _, prompt in
                if prompt != nil {
    ...
    private var rootContentWithPresentations: some View {
        rootContentWithLifecycleObservers
            // Presents exactly one root-owned sheet at a time so pairing, updates,
    ...
    private var pairingSetupBody: some View {
        PairingSetupView(
            onBack: isShowingPairingSetup ? { dismissPairingSetupToHome() } : nil,
            onConnect: { pairingPayload in
                Task {
                    isShowingPairingSetup = false
                    hasDismissedPairingSetup = false
        ...
        GeometryReader { proxy in
            ...
            ZStack(alignment: .leading) {
                ...
                ZStack(alignment: .leading) {
                    ...
                    if petCompanionStore.isEnabled {
    ...
    private var mainNavigationLayer: some View {
        NavigationStack(path: $navigationPath) {
            mainContent
                ...
                .navigationDestination(for: TerminalNavigationRoute.self) { _ in
                    SshScreen()
                        .adaptiveNavigationBar()
                }
        }
    ...
    private var mainContent: some View {
        if isOpeningNewChatFromSidebar {
            ...
            HomeEmptyStateView(
                ...
                offlinePrimaryButtonTitle: codex.hasReconnectCandidate ? "Reconnect" : "Connect",
                onPrimaryAction: {
                ...
                if homeConnectionPhase == .connecting || (codex.hasReconnectCandidate && !codex.isConnected) {
                    if shouldOfferWakeSavedMacDisplayAction {
                        ...
                        .buttonStyle(.plain)
                        .disabled(isWakingSavedMacDisplay)
                    }
    ...
    // Shows pairing setup on first launch or when the user explicitly requests a new pairing.
    private var shouldShowPairingSetup: Bool {
        guard !codex.isConnected else {
            return false
        }

        if isShowingPairingSetup {
            return true
        }

        if viewModel.isAttemptingAutoReconnect || shouldShowReconnectShell {
            return false
        }

        return !codex.hasReconnectCandidate && !hasDismissedPairingSetup
    }
    ...
    private func scheduleSidebarPrewarmIfNeeded() {
        guard scenePhase == .active,
            ...
            debugSidebarLog(
                "prewarm skipped phase=\(String(describing: scenePhase)) pairingSetup=\(isShowingPairingSetup) "
    ...
    // Blocks lower-priority sheets while pairing or root alerts own the screen.
    private var canPresentDeferredRootSheet: Bool {
        scenePhase == .active
            && !isShowingPairingSetup
            && !shouldShowPairingSetup
            && codex.missingNotificationThreadPrompt == nil
    }
    ...
    private var whatsNewPresentationScheduleFingerprint: String {
        [
            ...
            String(isShowingPairingSetup),
            String(shouldShowPairingSetup),
    ...
    private func bridgeUpdateSheet(prompt: CodexBridgeUpdatePrompt) -> some View {
        BridgeUpdateSheet(
            ...
            onNewPairing: {
                presentPairingSetupForBridgeRecovery()
            },
            onDismiss: {
                dismissPresentedRootSheet()
            }
    ...
    // Switches the user to the pairing setup when the old relay session is no longer useful.
    private func presentPairingSetupForBridgeRecovery() {
        guard !isShowingPairingSetup else {
            return
        }
    ...
    // Shows pairing setup immediately and tears down any stale reconnect in the background.
    private func presentPairingSetupAfterStoppingReconnect() {
        guard !isShowingPairingSetup else {
            return
        }
    ...
    // Re-opens the pairing setup after the user backed out to the empty state without a saved pairing.
    private func presentPairingSetup() {
        withAnimation {
            hasDismissedPairingSetup = false
        }
    }

    // Hides the pairing setup without forcing the user straight back into it on the next render pass.
    private func dismissPairingSetupToHome() {
        withAnimation {
            isShowingPairingSetup = false
            hasDismissedPairingSetup = true
        }
    }

    // Offers a new pairing path as a quiet secondary row under the main reconnect CTA.
    private var reconnectSecondaryActions: some View {
        secondaryReconnectActionButton("New Pairing") {
            presentPairingSetupAfterStoppingReconnect()
        }
    }
