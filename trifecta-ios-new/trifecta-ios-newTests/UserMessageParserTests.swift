...
final class UserMessageParserTests: XCTestCase {
    func testParseKeepsLeadingFileMentionWithSpaces() {
        let parsed = UserMessageParser.parse(
            "@Trifecta Mobile App Plan/Trifecta iOS Recap TLDR.md add other 2 lines"
        )

        XCTAssertEqual(parsed.mentions, ["Trifecta Mobile App Plan/Trifecta iOS Recap TLDR.md"])
    ...
    func testParseDoesNotTreatBareTerminalHandleAsFileMention() {
        let parsed = UserMessageParser.parse("@trifecta cache hit")

        XCTAssertEqual(parsed.mentions, [])
        XCTAssertEqual(parsed.body, "@trifecta cache hit")
    }
