...
final class CodexServiceTierTests: XCTestCase {
    ...
    func testKnownFastModelsUseStaticFallbackWhenRuntimeMetadataOmitsCapability() throws {
        ...
        [
          ...
          {
            "slug": "gpt-5.3-codex",
            "name": "GPT-5.3 Trifecta",
            "supportedReasoningEfforts": ["medium"]
          },
          {
            "slug": "gpt-5.3-codex-spark",
            "name": "GPT-5.3 Trifecta Spark",
            "supportedReasoningEfforts": ["medium"]
          },
          {
            "slug": "unknown-model",
    ...
    func testUnsupportedServiceTierDisablesFutureRetriesAndShowsUpdatePromptOnce() async throws {
        ...
        XCTAssertEqual(service.bridgeUpdatePrompt?.title, "Update Trifecta on your Mac to use Speed controls")
        XCTAssertEqual(
            service.bridgeUpdatePrompt?.message,
            "This Mac bridge does not support the selected speed setting yet. Update the Trifecta npm package to use Fast Mode and other speed controls."
        )
        XCTAssertEqual(service.bridgeUpdatePrompt?.command, "npm install -g trifecta@1.1.4")
    }
