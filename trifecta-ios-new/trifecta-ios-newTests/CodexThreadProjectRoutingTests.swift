...
final class CodexThreadProjectRoutingTests: XCTestCase {
    ...
    func testStartThreadIfReadyWaitsForRuntimeInitializationDuringReconnect() async throws {
        ...
        let thread = try await service.startThreadIfReady(preferredProjectPath: "/tmp/trifecta-local")
    ...
    func testMoveThreadToProjectPathKeepsRebindWhenResumeFailsOnlyBecauseRolloutIsMissing() async throws {
        ...
        let movedThread = try await service.moveThreadToProjectPath(
            threadId: "thread-1",
            projectPath: "/tmp/trifecta-worktree"
        )

        XCTAssertEqual(resumeRequests.count, 1)
        XCTAssertEqual(resumeRequests.first?["threadId"]?.stringValue, "thread-1")
        XCTAssertEqual(resumeRequests.first?["cwd"]?.stringValue, "/tmp/trifecta-worktree")
        XCTAssertEqual(movedThread.gitWorkingDirectory, "/tmp/trifecta-worktree")
        XCTAssertEqual(service.thread(for: "thread-1")?.gitWorkingDirectory, "/tmp/trifecta-worktree")
        XCTAssertEqual(service.currentAuthoritativeProjectPath(for: "thread-1"), "/tmp/trifecta-worktree")
    ...
    func testManagedWorktreeAssociationPersistsAcrossLocalHandoffs() async throws {
        ...
        let worktreePath = "/Users/me/.codex/worktrees/a1b2/trifecta"
        _ = try await service.moveThreadToProjectPath(threadId: "thread-1", projectPath: worktreePath)
        _ = try await service.moveThreadToProjectPath(threadId: "thread-1", projectPath: "/tmp/trifecta-local")

        XCTAssertEqual(resumeResponses, [worktreePath, "/tmp/trifecta-local"])
