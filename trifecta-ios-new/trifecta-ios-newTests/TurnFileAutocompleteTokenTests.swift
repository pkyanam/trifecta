...
final class TurnFileAutocompleteTokenTests: XCTestCase {
    ...
    func testTrailingTokenStopsAtWhitespaceAfterAtToken() {
        XCTAssertNil(
            TurnViewModel.trailingFileAutocompleteToken(
                in: "update @Trifecta Mobile App Plan/Trifecta iOS Recap TLDR.md"
            )
        )
    }
    ...
    func testTrailingTokenParsesBareLowercaseSearchAfterAt() {
        let token = TurnViewModel.trailingFileAutocompleteToken(in: "paste @trifecta")

        XCTAssertEqual(token?.query, "trifecta")
    }
    ...
    func testReplacingMentionAliasesNormalizesDifferentFilenameStyles() {
        let mention = TurnComposerMentionedFile(
            fileName: "Trifecta iOS Recap TLDR.md",
            path: "Trifecta Mobile App Plan/Trifecta iOS Recap TLDR.md"
        )
        let source = """
        ...
        XCTAssertTrue(replaced.contains("@Trifecta Mobile App Plan/Trifecta iOS Recap TLDR.md"))
    ...
    func testClosedConfirmedMentionSupportsFileNamesWithSpaces() {
        let mentions = [
            TurnComposerMentionedFile(
                fileName: "Trifecta iOS Recap TLDR.md",
                path: "Docs/Trifecta iOS Recap TLDR.md"
            ),
        ]

        XCTAssertTrue(
            TurnViewModel.hasClosedConfirmedFileMentionPrefix(
                in: "@Trifecta iOS Recap TLDR.md please revise this",
                confirmedMentions: mentions
            )
        )
    }
    ...
    func testTrailingAutocompleteClosesForOpenPathWithSpaces() {
        ...
        XCTAssertFalse(
            TurnViewModel.hasClosedConfirmedFileMentionPrefix(
                in: "compare @Trifecta Mobile App Plan/Trifecta iOS Recap TLDR.md",
                confirmedMentions: mentions
            )
        )
        XCTAssertNil(
