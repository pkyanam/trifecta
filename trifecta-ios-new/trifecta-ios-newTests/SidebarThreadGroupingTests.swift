...
final class SidebarThreadGroupingTests: XCTestCase {
    ...
    func testMakeGroupsMarksCodexManagedWorktreesInLabelAndIcon() throws {
        let now = Date(timeIntervalSince1970: 1_700_000_000)
        let threads = [
            makeThread(id: "main-thread", updatedAt: now, cwd: "/Users/me/work/Trifecta"),
            makeThread(
                id: "worktree-thread",
                updatedAt: now.addingTimeInterval(-60),
                cwd: "/Users/me/.codex/worktrees/ce15/Trifecta"
            ),
        ]

        let groups = SidebarThreadGrouping.makeGroups(from: threads, now: now)
        let mainGroup = try XCTUnwrap(groups.first(where: { $0.projectPath == "/Users/me/work/Trifecta" }))
        let worktreeGroup = try XCTUnwrap(
            groups.first(where: { $0.projectPath == "/Users/me/.codex/worktrees/ce15/Trifecta" })
        )

        XCTAssertEqual(mainGroup.label, "Trifecta")
        XCTAssertEqual(mainGroup.iconSystemName, "folder")
        XCTAssertEqual(worktreeGroup.label, "Trifecta 15")
    ...
    func testMakeProjectChoicesKeepWorktreeSelectionCompactWithoutShowingPathInLabel() {
        ...
        XCTAssertEqual(labelsByPath["/Users/me/work/Trifecta"]?.label, "Trifecta")
        XCTAssertEqual(labelsByPath["/Users/me/work/Trifecta"]?.iconSystemName, "folder")
        XCTAssertEqual(labelsByPath["/Users/me/.codex/worktrees/ce15/Trifecta"]?.label, "Trifecta 15")
        XCTAssertEqual(labelsByPath["/Users/me/.codex/worktrees/ce15/Trifecta"]?.iconSystemName, "arrow.triangle.branch")
    }
