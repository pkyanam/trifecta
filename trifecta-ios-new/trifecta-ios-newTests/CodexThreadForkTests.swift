...
final class CodexThreadForkTests: XCTestCase {
    ...
    func testForkSendsOnlyThreadIdToThreadFork() async throws {
            ...
            case "thread/resume":
                ...
                return RPCMessage(
                    ...
                    result: .object([
                        "thread": .object([
                            "id": .string("fork-local"),
                            "cwd": .string("/tmp/trifecta-worktree"),
                            "title": .string("Fork Local"),
                        ]),
                    ]),
                    includeJSONRPC: false
        ...
        let forkedThread = try await service.forkThreadIfReady(
            from: "source-thread",
            target: .projectPath("/tmp/trifecta-worktree")
        )
        ...
        XCTAssertEqual(capturedResumeParams["cwd"]?.stringValue, "/tmp/trifecta-worktree")
        XCTAssertEqual(forkedThread.id, "fork-local")
        XCTAssertEqual(forkedThread.gitWorkingDirectory, "/tmp/trifecta-worktree")
        XCTAssertEqual(service.activeThreadId, "fork-local")
    }
    ...
    func testUnsupportedThreadForkDisablesCapabilityAndShowsUpdatePrompt() async {
        ...
        do {
            ...
            XCTAssertEqual(service.bridgeUpdatePrompt?.title, "Update Trifecta on your Mac to use /fork")
        }
    }
    ...
    func testForkKeepsAuthoritativeProjectPathWhenResumeFallsBackToMissingRollout() async throws {
        ...
        XCTAssertEqual(forkedThread.gitWorkingDirectory, "/tmp/trifecta-worktree")
        XCTAssertEqual(service.currentAuthoritativeProjectPath(for: "fork-local"), "/tmp/trifecta-worktree")
        ...
        XCTAssertEqual(service.thread(for: "fork-local")?.gitWorkingDirectory, "/tmp/trifecta-worktree")
        XCTAssertEqual(service.currentAuthoritativeProjectPath(for: "fork-local"), "/tmp/trifecta-worktree")
