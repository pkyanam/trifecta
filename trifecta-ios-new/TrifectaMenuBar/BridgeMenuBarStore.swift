...
enum BridgeMenuBarActionError: LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .missingCLI:
            return "Install the global `trifecta` CLI before using this companion."
...
final class BridgeMenuBarStore: ObservableObject {
    ...
    private static let relayOverrideKey = "trifecta.menuBar.relayOverride"
    ...
    func resumeLastThread() {
        runAction(successMessage: "Ultimo thread riaperto in Trifecta.") {
            try await self.requireCLIAvailability()
    ...
    func openLogsFolder() {
        ...
        let targetPath = (path?.isEmpty == false) ? path! : "\(NSHomeDirectory())/.trifecta"
