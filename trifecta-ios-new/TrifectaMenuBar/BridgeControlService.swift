...
// Purpose: Wraps the existing trifecta/npm shell commands so the menu bar app can detect the global CLI and control the bridge.
...
struct BridgeCLIInvocation {
    ...
    func command(_ arguments: [String]) -> String {
        ([shellQuoted(nodePath), shellQuoted(trifectaPath)] + arguments).joined(separator: " ")
    }
}
...
final class BridgeControlService {
    ...
    private let defaultStateDirectory = URL(fileURLWithPath: NSHomeDirectory())
        .appendingPathComponent(".trifecta", isDirectory: true)
    private let launchAgentPlistURL = URL(fileURLWithPath: NSHomeDirectory())
        .appendingPathComponent("Library", isDirectory: true)
        .appendingPathComponent("LaunchAgents", isDirectory: true)
        .appendingPathComponent("com.trifecta.bridge.plist")

    init(runner: ShellCommandRunner = ShellCommandRunner()) {
        self.runner = runner
    }

    // Confirms the product contract for this companion: a global `trifecta` CLI must be runnable first.
    ...
    func updateBridgePackage() async throws {
        _ = try await runner.run(command: "npm install -g trifecta@latest")
    }

    func fetchLatestPackageVersion() async -> Result<String, Error> {
        do {
            let result = try await runner.run(command: "npm view trifecta version --json")
            let latestVersion = parseLatestVersion(result.stdout)
            guard let latestVersion else {
                throw BridgeControlError.commandFailed(
                    command: "npm view trifecta version --json",
    ...
    private func loadFallbackSnapshot(
        ...
        return BridgeSnapshot(
            ...
            label: statusLines["service label"] ?? "com.trifecta.bridge",
            platform: "darwin",
            installed: installed,
    ...
    private func resolveCLIInvocation() async throws -> BridgeCLIInvocation {
        let trifectaPath = try await resolveExecutable(named: "trifecta")
        let nodePath = try await resolveNodePath(for: trifectaPath)
        return BridgeCLIInvocation(nodePath: nodePath, trifectaPath: trifectaPath)
    }
    ...
    private func resolveNodePath(for trifectaPath: String) async throws -> String {
        if let colocatedNodePath = resolveColocatedNodePath(for: trifectaPath) {
    ...
    private func resolveColocatedNodePath(for trifectaPath: String) -> String? {
        let trifectaURL = URL(fileURLWithPath: trifectaPath)
        let candidateDirectories = [
            trifectaURL.deletingLastPathComponent().path,
            trifectaURL.resolvingSymlinksInPath().deletingLastPathComponent().path,
        ]
    ...
    private func resolveStateDirectory(statusLines: [String: String]) -> URL {
        if let explicitStateDirectory = normalizeNonEmptyString(ProcessInfo.processInfo.environment["TRIFECTA_DEVICE_STATE_DIR"]) {
    ...
    private func readLaunchAgentStateDirectory() -> URL? {
        guard let data = try? Data(contentsOf: launchAgentPlistURL),
              ...
              let stateDirectory = normalizeNonEmptyString(environment["TRIFECTA_DEVICE_STATE_DIR"] as? String) else {
            return nil
        }
    ...
    private func classifyCLIAvailability(from error: Error) -> BridgeCLIAvailability {
        ...
        if normalized.contains("command not found: trifecta")
            || normalized.contains("trifecta: command not found")
            || normalized.contains("trifecta: not found")
    ...
    private func commandEnvironment(relayOverride: String?) -> [String: String] {
        ...
        return [
            "TRIFECTA_RELAY": relayOverride.trimmingCharacters(in: .whitespacesAndNewlines),
        ]
    }
}
