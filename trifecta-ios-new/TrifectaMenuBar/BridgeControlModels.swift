...
enum BridgeCLIAvailability: Equatable {
    ...
    static let installCommand = "npm install -g trifecta@latest"
    ...
    var setupMessage: String {
        switch self {
        case .checking:
            return "Looking for a globally installed `trifecta` command before enabling the companion controls."
        case .available(let version):
            return "Detected the global `trifecta` CLI (`\(version)`)."
        case .missing:
            return "This companion only works when the global `trifecta` CLI is installed and visible to the app shell environment."
        case .broken(let message):
            return "Found `trifecta`, but the CLI could not be used. \(message)"
        }
    }
}
