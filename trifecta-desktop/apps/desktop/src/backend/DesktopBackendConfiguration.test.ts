    ...
    Layer.provide(
      Layer.mergeAll(
        ...
        DesktopConfig.layerTest({
          ...
          TRIFECTA_DESKTOP_LAN_HOST: "192.168.1.50",
  ...
  it.effect("resolves backend start config with a stable scoped bootstrap token", () =>
    withHarness(
      Effect.gen(function* () {
        ...
        assert.isUndefined(first.env.TRIFECTA_PORT);
        assert.isUndefined(first.env.TRIFECTA_MODE);
        assert.isUndefined(first.env.TRIFECTA_DESKTOP_LAN_HOST);
