...
const defaultInput = {
  ...
  processArch: "arm64",
  appVersion: "0.0.22",
  appPath: "/Applications/Belweave.app/Contents/Resources/app.asar",
  isPackaged: false,
  resourcesPath: "/Applications/Belweave.app/Contents/Resources",
...
describe("DesktopEnvironment", () => {
  it.effect("derives state paths and development identity inside Effect", () =>
    Effect.gen(function* () {
      const environment = yield* makeEnvironment(
        {},
        {
          BELWEAVE_HOME: " /tmp/belweave ",
          BELWEAVE_COMMIT_HASH: " 0123456789abcdef ",
          BELWEAVE_PORT: "4949",
          VITE_DEV_SERVER_URL: "http://localhost:5173",
          BELWEAVE_DEV_REMOTE_BELWEAVE_SERVER_ENTRY_PATH: " /remote/server.mjs ",
          BELWEAVE_OTLP_TRACES_URL: " http://127.0.0.1:4318/v1/traces ",
          BELWEAVE_OTLP_EXPORT_INTERVAL_MS: "2500",
        },
      );

      assert.equal(environment.isDevelopment, true);
      assert.equal(environment.appDataDirectory, "/Users/alice/Library/Application Support");
      assert.equal(environment.baseDir, "/tmp/belweave");
      ...
      assert.equal(environment.appUserModelId, "com.belweave.belweave.dev");
      assert.equal(environment.linuxWmClass, "belweave-dev");
      assert.deepEqual(
  ...
  it.effect("derives production state paths under userdata", () =>
    Effect.gen(function* () {
      ...
      assert.equal(environment.stateDir, "/tmp/belweave/userdata");
      assert.equal(environment.logDir, "/tmp/belweave/userdata/logs");
      assert.equal(environment.serverSettingsPath, "/tmp/belweave/userdata/settings.json");
    }),
  );
