...
const makeDesktopBootstrap = (
  ...
  noBrowser: true,
  port: 4888,
  belweaveHome: "/tmp/belweave-bootstrap-home",
  host: "127.0.0.1",
...
it.layer(NodeServices.layer)("cli config resolution", (it) => {
  const defaultObservabilityConfig = {
    ...
    otlpServiceName: "belweave-server",
  } as const;

  const openBootstrapFd = Effect.fn(function* (payload: DesktopBackendBootstrapValue) {
    const fs = yield* FileSystem.FileSystem;
    const filePath = yield* fs.makeTempFileScoped({ prefix: "belweave-bootstrap-", suffix: ".ndjson" });
  ...
  it.effect("falls back to effect/config values when flags are omitted", () =>
    Effect.gen(function* () {
      ...
      const baseDir = join(NodeOS.tmpdir(), "belweave-cli-config-env-base");
          ...
          Layer.mergeAll(
            ConfigProvider.layer(
              ConfigProvider.fromEnv({
                env: {
                  BELWEAVE_LOG_LEVEL: "Warn",
                  BELWEAVE_MODE: "desktop",
                  BELWEAVE_PORT: "4001",
                  BELWEAVE_HOST: "0.0.0.0",
                  BELWEAVE_HOME: baseDir,
                  VITE_DEV_SERVER_URL: "http://127.0.0.1:5173",
                  BELWEAVE_NO_BROWSER: "true",
                  BELWEAVE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD: "false",
                  BELWEAVE_LOG_WS_EVENTS: "true",
                },
              }),
            ),
            NetService.layer,
  ...
  it.effect("uses CLI flags when provided", () =>
    Effect.gen(function* () {
      const { join } = yield* Path.Path;
      const baseDir = join(NodeOS.tmpdir(), "belweave-cli-config-flags-base");
            ...
            ConfigProvider.layer(
              ConfigProvider.fromEnv({
                env: {
                  ...
                  BELWEAVE_PORT: "4001",
                  BELWEAVE_HOST: "0.0.0.0",
                  BELWEAVE_HOME: join(NodeOS.tmpdir(), "ignored-base"),
                  VITE_DEV_SERVER_URL: "http://127.0.0.1:5173",
                  BELWEAVE_NO_BROWSER: "false",
                  BELWEAVE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD: "false",
                  BELWEAVE_LOG_WS_EVENTS: "false",
                },
              }),
            ),
            NetService.layer,
  ...
  it.effect("preserves explicit false CLI boolean flags over env and bootstrap values", () =>
    Effect.gen(function* () {
      const { join } = yield* Path.Path;
      const baseDir = join(NodeOS.tmpdir(), "belweave-cli-config-false-flags");
        ...
        Effect.provide(
          Layer.mergeAll(
            ConfigProvider.layer(
              ConfigProvider.fromEnv({
                env: {
                  BELWEAVE_BOOTSTRAP_FD: String(fd),
                  BELWEAVE_NO_BROWSER: "true",
                  BELWEAVE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD: "true",
                  BELWEAVE_LOG_WS_EVENTS: "true",
                },
              }),
            ),
  ...
  it.effect("uses bootstrap envelope values as fallbacks when flags and env are absent", () =>
    Effect.gen(function* () {
      const { join } = yield* Path.Path;
      const baseDir = "/tmp/belweave-bootstrap-home";
      const fd = yield* openBootstrapFd(
        makeDesktopBootstrap({
          port: 4888,
          host: "127.0.0.2",
          belweaveHome: baseDir,
          noBrowser: true,
  ...
  it.effect("creates derived runtime directories during config resolution", () =>
    Effect.gen(function* () {
      ...
      const baseDir = yield* fs.makeTempDirectoryScoped({ prefix: "belweave-cli-config-dirs-" });
  ...
  it.effect("applies flag then env precedence over bootstrap envelope values", () =>
    Effect.gen(function* () {
      const { join } = yield* Path.Path;
      const baseDir = join(NodeOS.tmpdir(), "belweave-cli-config-env-wins");
      const fd = yield* openBootstrapFd(
        makeDesktopBootstrap({
          port: 4888,
          host: "127.0.0.2",
  ...
  it.effect("falls back to persisted observability settings when env vars are absent", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const baseDir = yield* fs.makeTempDirectoryScoped({ prefix: "belweave-cli-config-settings-" });
      const derivedPaths = yield* deriveServerPaths(baseDir, undefined);
  ...
  it.effect("forces noBrowser and disables auto-bootstrap for headless startup presentation", () =>
    Effect.gen(function* () {
      const { join } = yield* Path.Path;
      const baseDir = join(NodeOS.tmpdir(), "belweave-cli-config-headless-base");
      const derivedPaths = yield* deriveServerPaths(baseDir, undefined);
