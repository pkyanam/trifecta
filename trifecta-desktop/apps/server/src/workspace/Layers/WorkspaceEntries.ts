...
import { type FilesystemBrowseInput, type ProjectEntry } from "@belweave/contracts";
import { isExplicitRelativePath, isWindowsAbsolutePath } from "@belweave/shared/path";
import {
  insertRankedSearchResult,
  normalizeSearchQuery,
  scoreQueryMatch,
  type RankedSearchResult,
} from "@belweave/shared/searchRanking";
...
export const makeWorkspaceEntries = Effect.gen(function* () {
  ...
  const list: WorkspaceEntriesShape["list"] = Effect.fn("WorkspaceEntries.list")(function* (input) {
    ...
    return yield* Cache.get(workspaceIndexCache, normalizedCwd).pipe(
      Effect.map((index) => ({
        entries: index.entries.map(({ path, kind, parentPath }) => ({
          path,
          kind,
