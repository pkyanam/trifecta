/**
 * GrokDriver — `ProviderDriver` for the Grok CLI (`grok`) runtime.
 *
 * Grok exposes an ACP-based CLI with xAI-specific extensions. The driver
 * mirrors `CursorDriver` but uses `makeManualOnlyProviderMaintenanceCapabilities`
 * since Grok does not yet expose a programmatic auto-updater.
 *
 * Text generation is supported via the ACP runtime — `makeGrokTextGeneration`
 ...
 */
import { GrokSettings, ProviderDriverKind, type ServerProvider } from "@belweave/contracts";
