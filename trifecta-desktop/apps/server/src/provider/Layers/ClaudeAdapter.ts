...
import { parseCliArgs } from "@belweave/shared/cliArgs";
import {
  ApprovalRequestId,
  ...
  const handleSdkTelemetryMessage = Effect.fn("handleSdkTelemetryMessage")(function* (
    ...
    if (message.type === "rate_limit_event") {
      ...
      yield* offerRuntimeEvent({
        ...
        payload: {
          rateLimits: {
            limitName: info.rateLimitType ?? null,
            primary:
              info.utilization !== undefined || info.resetsAt !== undefined
                ? {
                    usedPercent: info.utilization ?? 0,
  ...
  const startSession: ClaudeAdapterShape["startSession"] = Effect.fn("startSession")(
    function* (input) {
      ...
      const handleAskUserQuestion = Effect.fn("handleAskUserQuestion")(function* (
        ...
        // expected lookup key. See https://github.com/belweave/trifecta/issues/2388
