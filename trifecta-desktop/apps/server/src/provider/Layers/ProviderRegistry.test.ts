...
import { deepMerge } from "@belweave/shared/Struct";
...
process.env.BELWEAVE_CURSOR_ENABLED = "1";
...
it.layer(
  ...
  describe("ProviderRegistryLive", () => {
          ...
          ProviderRegistryLive.pipe(
            ...
            Layer.provideMerge(
              ServerConfig.layerTest(process.cwd(), {
                prefix: "belweave-provider-registry-merged-persist-",
              }),
            ),
      ...
      Effect.gen(function* () {
        ...
        const runtimeServices = yield* Layer.build(
          ProviderRegistryLive.pipe(
            ...
            Layer.provideMerge(
              ServerConfig.layerTest(process.cwd(), {
                prefix: "belweave-provider-registry-refresh-failure-",
              }),
            ),
            Layer.provideMerge(NodeServices.layer),
          ),
      ...
      Effect.gen(function* () {
        ...
        const runtimeServices = yield* Layer.build(
          ProviderRegistryLive.pipe(
            ...
            Layer.provideMerge(
              ServerConfig.layerTest(process.cwd(), {
                prefix: "belweave-provider-registry-sync-failure-",
              }),
            ),
            Layer.provideMerge(NodeServices.layer),
          ),
    ...
    it.effect("propagates real Codex probe failures to the aggregator at boot", () =>
      Effect.gen(function* () {
        const missingBinary = `belweave_codex_missing_`;
        const serverSettings = yield* makeMutableServerSettingsService(
          decodeServerSettings(
            deepMerge(encodedDefaultServerSettings, {
              ...
              providerInstances: {
                // Matches the shape the user had in `.belweave/dev/settings.json`
        ...
        const providerRegistryLayer = ProviderRegistryLive.pipe(
          ...
          Layer.provideMerge(
            ServerConfig.layerTest(process.cwd(), {
              prefix: "belweave-provider-registry-",
            }),
          ),
        ...
        yield* Effect.gen(function* () {
          ...
          for (
            let attempt = 0;
            attempt < 120 && codexPersonal?.status === "warning";
            attempt += 1
          ) {
    ...
    it.effect("re-probes when settings change the codex binaryPath", () =>
      Effect.gen(function* () {
        const firstMissing = `belweave_codex_first_`;
        const secondMissing = `belweave_codex_second_`;
        const serverSettings = yield* makeMutableServerSettingsService(
          decodeServerSettings(
            deepMerge(encodedDefaultServerSettings, {
              providers: {
    ...
    it.effect("keeps cursor disabled and skips probing when the provider setting is disabled", () =>
      Effect.gen(function* () {
        ...
        yield* Effect.gen(function* () {
          ...
          assert.strictEqual(cursorProvider?.message, "Cursor is disabled in Trifecta settings.");
