...
const TelemetryEnvConfig = Config.all({
  posthogKey: Config.string("TRIFECTA_POSTHOG_KEY").pipe(
    Config.withDefault("phc_XOWci4oZP4VvLiEyrFqkFjP4CZn55mjYYBMREK5Wd6m"),
  ),
  posthogHost: Config.string("TRIFECTA_POSTHOG_HOST").pipe(
    Config.withDefault("https://us.i.posthog.com"),
  ),
  enabled: Config.boolean("TRIFECTA_TELEMETRY_ENABLED").pipe(Config.withDefault(true)),
  flushBatchSize: Config.number("TRIFECTA_TELEMETRY_FLUSH_BATCH_SIZE").pipe(Config.withDefault(20)),
  maxBufferedEvents: Config.number("TRIFECTA_TELEMETRY_MAX_BUFFERED_EVENTS").pipe(
    ...
    const payload = {
      ...
      batch: events.map((event) => ({
        ...
        properties: {
          ...
          arch: process.arch,
          trifectaVersion: packageJson.version,
          clientType,
        },
