...
const BitbucketApiEnvConfig = Config.all({
  baseUrl: Config.string("TRIFECTA_BITBUCKET_API_BASE_URL").pipe(
    Config.withDefault(DEFAULT_API_BASE_URL),
  ),
  accessToken: Config.string("TRIFECTA_BITBUCKET_ACCESS_TOKEN").pipe(Config.option),
  email: Config.string("TRIFECTA_BITBUCKET_EMAIL").pipe(Config.option),
  apiToken: Config.string("TRIFECTA_BITBUCKET_API_TOKEN").pipe(Config.option),
});
...
function checkoutBranchName(input: {
  ...
  return `trifecta/pr-${input.pullRequestId}/${sanitizeBranchFragment(input.headBranch)}`;
}
...
function authFromConfig(
  ...
  return {
    ...
    host: Option.some("bitbucket.org"),
    detail: Option.some(
      "Set TRIFECTA_BITBUCKET_EMAIL and TRIFECTA_BITBUCKET_API_TOKEN, or TRIFECTA_BITBUCKET_ACCESS_TOKEN.",
    ),
  };
}
