...
it.layer(NodeServices.layer)("launchDetached", (it) => {
  ...
  it.effect("rejects when command does not exist", () =>
    Effect.gen(function* () {
      const result = yield* launchDetached({
        command: `belweave-no-such-command-${yield* Random.nextUUIDv4}`,
        args: [],
      }).pipe(Effect.result);
...
it.layer(NodeServices.layer)("isCommandAvailable", (it) => {
  ...
  it.effect("uses platform-specific PATH delimiter for platform overrides", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const firstDir = yield* fs.makeTempDirectoryScoped({ prefix: "belweave-open-test-" });
      const secondDir = yield* fs.makeTempDirectoryScoped({ prefix: "belweave-open-test-" });
