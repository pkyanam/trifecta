...
import { type GrokSettings, type ModelSelection, TextGenerationError } from "@belweave/contracts";
import { sanitizeBranchFragment, sanitizeFeatureBranchName } from "@belweave/shared/git";
import { extractJsonObject } from "@belweave/shared/schemaJson";
...
/**
 * Build a Grok text-generation closure bound to a specific `GrokSettings`
 * payload. Mirrors `makeCursorTextGeneration` but drives Grok's ACP runtime
 * with xAI prompt-completion fallback support.
 */
  ...
  const runGrokJson = <S extends Schema.Top>({
    ...
    Effect.gen(function* () {
      ...
      const runtime = yield* makeGrokAcpRuntime({
        ...
        cwd,
        clientInfo: { name: "trifecta-git-text", version: "0.0.0" },
      });
