...
const HOSTED_WEB_CHANNEL_COOKIE = "belweave_web_channel";
...
export const config: VercelConfig = {
  buildCommand:
    'turbo build --filter @belweave/web && bun ../../scripts/apply-web-brand-assets.ts --channel "${VITE_HOSTED_APP_CHANNEL:-latest}"',
  git: {
    deploymentEnabled: false,
  },
  installCommand:
    "bun add -g turbo && bun install --filter '@belweave/contracts' --filter '@belweave/client-runtime' --filter '@belweave/scripts' --filter '@belweave/web'",
  routes: [
    {
      src: "/__belweave/channel",
