...
import { scopedThreadKey, scopeThreadRef } from "@belweave/client-runtime";
import { createModelCapabilities, createModelSelection } from "@belweave/shared/model";
...
import { DEFAULT_CLIENT_SETTINGS } from "@belweave/contracts/settings";
...
function createBaseServerConfig(): ServerConfig {
  return {
    ...
    auth: {
      ...
      sessionCookieName: "belweave_session",
    },
    cwd: "/repo/project",
    keybindingsConfigPath: "/repo/project/.belweave-keybindings.json",
    keybindings: [],
    issues: [],
    providers: [
      {
    ...
    availableEditors: [],
    observability: {
      logsDirectoryPath: "/repo/project/.belweave/logs",
      ...
      sourceControlProviders: [
        {
          ...
          auth: {
            ...
            account: Option.some("belweave-oss"),
...
describe("ChatView timeline estimator parity (full app)", () => {
      ...
      await vi.waitFor(
        () => {
          ...
          expect(openRequest).toMatchObject({
            ...
            worktreePath: null,
            env: {
              BELWEAVE_PROJECT_ROOT: "/repo/project",
            },
          });
          expect(openRequest?.env?.BELWEAVE_WORKTREE_PATH).toBeUndefined();
        },
        { timeout: 8_000, interval: 16 },
      );
    } finally {
  ...
  it("falls back to the first installed editor when the stored favorite is unavailable", async () => {
    localStorage.setItem("belweave:last-editor", JSON.stringify("vscodium"));
    setDraftThreadWithoutWorktree();
      ...
      await vi.waitFor(
        () => {
          ...
          expect(openRequest).toMatchObject({
            ...
            env: {
              BELWEAVE_PROJECT_ROOT: "/repo/project",
              BELWEAVE_WORKTREE_PATH: "/repo/worktrees/feature-draft",
            },
          });
        },
        { timeout: 8_000, interval: 16 },
        ...
        if (body._tag === WS_METHODS.gitResolvePullRequest) {
          return {
            pullRequest: {
              ...
              url: "https://github.com/belweave/trifecta/pull/1359",
              baseBranch: "main",
          ...
          expect(dispatchRequest).toMatchObject({
            ...
            bootstrap: {
              ...
              prepareWorktree: {
                ...
                baseBranch: "main",
                branch: expect.stringMatching(/^belweave\/[0-9a-f]{8}$/),
              },
              runSetupScript: true,
            },
          });
  ...
  it("shows the confirm archive action after clicking the archive button", async () => {
    ...
    try {
      ...
      localStorage.removeItem("belweave:client-settings:v1");
      await mounted.cleanup();
    }
  });
        ...
        threads: createSnapshotForTargetUser({
          ...
          thread.id === THREAD_ID
            ? Object.assign({}, thread, {
                ...
                worktreePath: "/repo/.belweave/worktrees/existing",
              })
            : thread,
        ),
      },
    ...
    const mounted = await mountChatView({
      ...
      resolveRpc: (body) => {
        ...
        if (body._tag === WS_METHODS.sourceControlLookupRepository) {
          return {
            provider: "github",
            nameWithOwner: "belweave-oss/belweave-env",
            url: "https://github.com/belweave-oss/belweave-env",
            sshUrl: "git@github.com:belweave-oss/belweave-env.git",
          };
        }
      ...
      await vi.waitFor(
        () => {
          ...
          expect(cloneRequest).toMatchObject({
            remoteUrl: "git@github.com:belweave-oss/belweave-env.git",
  ...
  it("keeps the wide desktop follow-up layout expanded when the footer still fits", async () => {
    const mounted = await mountChatView({
      ...
      snapshot: createSnapshotWithPlanFollowUpPrompt({
        ...
        planMarkdown:
          "# Imaginary Long-Range Plan: Belweave Adaptive Orchestration and Safe-Delay Execution Initiative",
      }),
    });
