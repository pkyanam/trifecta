...
describe("GeneralSettingsPanel observability", () => {
  ...
  it("shows a disabled network access toggle with guidance in desktop builds", async () => {
    ...
    await expect
      .element(page.getByText("Trifecta will restart to expose this environment over the network."))
      .toBeInTheDocument();
  ...
  it("opens the logs folder in the preferred editor", async () => {
    ...
    window.nativeApi = {
      ...
      server: {
        ...
        getTraceDiagnostics: vi.fn().mockResolvedValue({
          traceFilePath: "/repo/project/.belweave/traces.jsonl",
          scannedFilePaths: ["/repo/project/.belweave/traces.jsonl"],
          readAt: makeUtc("2036-04-07T00:00:00.000Z"),
          recordCount: 0,
          parseErrorCount: 0,
    ...
    expect(openInEditor).toHaveBeenCalledWith("/repo/project/.belweave/logs", "cursor");
  });
