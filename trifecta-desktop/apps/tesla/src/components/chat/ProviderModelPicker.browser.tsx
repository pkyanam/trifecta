import { ProviderDriverKind, ProviderInstanceId, type ServerProvider } from "@belweave/contracts";
import { EnvironmentId } from "@belweave/contracts";
...
describe("ProviderModelPicker", () => {
  ...
  it("enables vertical touch scrolling for Tesla provider and model lists", async () => {
    document.documentElement.classList.add("tesla-dashboard");
    const mounted = await mountPicker({
      activeInstanceId: CLAUDE_INSTANCE_ID,
      model: "claude-opus-4-6",
      lockedProvider: null,
    ...
    try {
      ...
      await vi.waitFor(() => {
        const sidebarViewport = document.querySelector<HTMLElement>(
          '[data-model-picker-sidebar="true"] [data-slot="scroll-area-viewport"]',
        );
        const modelsViewport = document.querySelector<HTMLElement>(
          '[data-model-picker-models="true"] [data-slot="scroll-area-viewport"]',
        );
        expect(sidebarViewport).not.toBeNull();
        expect(modelsViewport).not.toBeNull();
        expect(getComputedStyle(sidebarViewport!).touchAction).toBe("pan-y");
        expect(getComputedStyle(sidebarViewport!).overscrollBehaviorY).toBe("contain");
        expect(getComputedStyle(modelsViewport!).touchAction).toBe("pan-y");
        expect(getComputedStyle(modelsViewport!).overscrollBehaviorY).toBe("contain");
      });
    } finally {
      document.documentElement.classList.remove("tesla-dashboard");
      await mounted.cleanup();
    }
  });
