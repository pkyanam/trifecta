...
interface StagePackageJson {
  ...
  readonly belweaveCommitHash: string;
...
const BuildEnvConfig = Config.all({
  platform: Config.schema(BuildPlatform, "BELWEAVE_DESKTOP_PLATFORM").pipe(Config.option),
  target: Config.string("BELWEAVE_DESKTOP_TARGET").pipe(Config.option),
  arch: Config.schema(BuildArch, "BELWEAVE_DESKTOP_ARCH").pipe(Config.option),
  version: Config.string("BELWEAVE_DESKTOP_VERSION").pipe(Config.option),
  outputDir: Config.string("BELWEAVE_DESKTOP_OUTPUT_DIR").pipe(Config.option),
  skipBuild: Config.boolean("BELWEAVE_DESKTOP_SKIP_BUILD").pipe(Config.withDefault(false)),
  keepStage: Config.boolean("BELWEAVE_DESKTOP_KEEP_STAGE").pipe(Config.withDefault(false)),
...
function resolveGitHubPublishConfig(updateChannel: "latest" | "nightly"):
  ...
  | undefined {
  const rawRepo =
    process.env.BELWEAVE_DESKTOP_UPDATE_REPOSITORY?.trim() ||
...
const buildDesktopArtifact = Effect.fn("buildDesktopArtifact")(function* (
  ...
  const stagePackageJson: StagePackageJson = {
    ...
    belweaveCommitHash: commitHash,
    private: true,
    description: "Trifecta desktop build",
    author: "Preetham Kyanam <kyanam.preetham@gmail.com>",
    homepage: "https://trifecta.belweave.ai",
...
const buildDesktopArtifactCli = Command.make("build-desktop-artifact", {
  platform: Flag.choice("platform", BuildPlatform.literals).pipe(
    Flag.withDescription("Build platform (env: BELWEAVE_DESKTOP_PLATFORM)."),
    Flag.optional,
  ),
  target: Flag.string("target").pipe(
    Flag.withDescription(
      "Artifact target, for example dmg/AppImage/nsis (env: BELWEAVE_DESKTOP_TARGET).",
    ),
    Flag.optional,
  ),
  ...
  outputDir: Flag.string("output-dir").pipe(
    Flag.withDescription("Output directory for artifacts (env: BELWEAVE_DESKTOP_OUTPUT_DIR)."),
    Flag.optional,
  ),
  skipBuild: Flag.boolean("skip-build").pipe(
    Flag.withDescription(
      "Skip `bun run build:desktop` and use existing dist artifacts (env: BELWEAVE_DESKTOP_SKIP_BUILD).",
    ),
    Flag.optional,
  ),
  keepStage: Flag.boolean("keep-stage").pipe(
    Flag.withDescription("Keep temporary staging files (env: BELWEAVE_DESKTOP_KEEP_STAGE)."),
    Flag.optional,
  ),
  signed: Flag.boolean("signed").pipe(
    Flag.withDescription(
      "Enable signing/notarization discovery; Windows uses Azure Trusted Signing (env: BELWEAVE_DESKTOP_SIGNED).",
    ),
    Flag.optional,
  ),
  verbose: Flag.boolean("verbose").pipe(
    Flag.withDescription("Stream subprocess stdout (env: BELWEAVE_DESKTOP_VERBOSE)."),
    Flag.optional,
  ),
  mockUpdates: Flag.boolean("mock-updates").pipe(
    Flag.withDescription("Enable mock updates (env: BELWEAVE_DESKTOP_MOCK_UPDATES)."),
    Flag.optional,
  ),
