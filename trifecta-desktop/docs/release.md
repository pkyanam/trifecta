...
- Publishes the CLI package (`apps/server`, npm package `@belweave/trifecta`) with OIDC trusted publishing from the same workflow file:
...
- `BELWEAVE_WEB_ROUTER_URL`: defaults to `https://app.trifecta.belweave.ai`.
- `BELWEAVE_WEB_LATEST_DOMAIN`: defaults to `latest.app.trifecta.belweave.ai`.
- `BELWEAVE_WEB_NIGHTLY_DOMAIN`: defaults to `nightly.app.trifecta.belweave.ai`.
...
visiting `/__belweave/channel?channel=latest` or
`/__belweave/channel?channel=nightly`; the router stores the
`belweave_web_channel` cookie and rewrites future requests on `app.trifecta.belweave.ai` to
...
through `/__belweave/channel` on the router domain so the user's channel cookie is
...
- Publishes the CLI package (`apps/server`, npm package `@belweave/trifecta`) to the `nightly` npm dist-tag using the same nightly version.
...
- Temporary private-repo auth workaround:
  - set `BELWEAVE_DESKTOP_UPDATE_GITHUB_TOKEN` (or `GH_TOKEN`) in the desktop app runtime environment.
...
1. Confirm npm org/user owns package `@belweave/trifecta` (or rename package first if needed).
