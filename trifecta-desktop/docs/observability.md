...
```bash
export TRIFECTA_OTLP_TRACES_URL=http://localhost:4318/v1/traces
export TRIFECTA_OTLP_METRICS_URL=http://localhost:4318/v1/metrics
export TRIFECTA_OTLP_SERVICE_NAME=t3-local
```

Optional:

```bash
export TRIFECTA_TRACE_MIN_LEVEL=Info
export TRIFECTA_TRACE_TIMING_ENABLED=true
```
...
Launch the actual app executable from the same shell so the desktop app and embedded backend inherit `TRIFECTA_OTLP_*`.

macOS app bundle example:

```bash
TRIFECTA_OTLP_TRACES_URL=http://localhost:4318/v1/traces \
TRIFECTA_OTLP_METRICS_URL=http://localhost:4318/v1/metrics \
TRIFECTA_OTLP_SERVICE_NAME=t3-desktop \
...
```bash
tail -f "$TRIFECTA_HOME/userdata/logs/server.trace.ndjson"
```
...
jq -c 'select(.exit._tag != "Success") | {
  ...
  exit,
  attributes
}' "$TRIFECTA_HOME/userdata/logs/server.trace.ndjson"
```
...
jq -r 'select(.traceId == "TRACE_ID_HERE") | [
  ...
  (.parentSpanId // "-"),
  .durationMs
] | @tsv' "$TRIFECTA_HOME/userdata/logs/server.trace.ndjson"
```
...
- `TRIFECTA_OTLP_TRACES_URL` was not set
...
- `TRIFECTA_TRACE_FILE`: override trace file path
- `TRIFECTA_TRACE_MAX_BYTES`: per-file rotation size, default `10485760`
- `TRIFECTA_TRACE_MAX_FILES`: rotated file count, default `10`
- `TRIFECTA_TRACE_BATCH_WINDOW_MS`: flush window, default `200`
- `TRIFECTA_TRACE_MIN_LEVEL`: minimum trace level, default `Info`
- `TRIFECTA_TRACE_TIMING_ENABLED`: enable timing metadata, default `true`

OTLP export:

- `TRIFECTA_OTLP_TRACES_URL`: OTLP trace endpoint
- `TRIFECTA_OTLP_METRICS_URL`: OTLP metric endpoint
- `TRIFECTA_OTLP_EXPORT_INTERVAL_MS`: export interval, default `10000`
- `TRIFECTA_OTLP_SERVICE_NAME`: service name, default `t3-server`
