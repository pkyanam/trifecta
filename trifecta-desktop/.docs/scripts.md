...
- Dev commands default `TRIFECTA_STATE_DIR` to `~/.t3/dev` to keep dev state isolated from desktop/prod state.
...
Set `TRIFECTA_DEV_INSTANCE` to any value to deterministically shift all dev ports together.

- Default ports: server `3773`, web `5733`
- Shifted ports: `base + offset` (offset is hashed from `TRIFECTA_DEV_INSTANCE`)
- Example: `TRIFECTA_DEV_INSTANCE=branch-a bun run dev:desktop`

If you want full control instead of hashing, set `TRIFECTA_PORT_OFFSET` to a numeric offset.
