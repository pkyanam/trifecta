...
Use this when you want the desktop app to start or reuse Trifecta on another machine over SSH.
...
The remote host must have a compatible Node.js runtime. Trifecta uses the server package's `engines.node` requirement:

```text
^22.16 || ^23.11 || >=24.10
```

During SSH launch, Trifecta first checks whether `node` is already available on `PATH`. If it is missing, the launcher tries common non-interactive shell locations and version-manager shims/activation hooks:
...
If launch fails with `node: command not found`, a port-scan failure, or a message that the remote Node version does not satisfy the required range, SSH into the host and check the same non-interactive shell path Trifecta uses:
...
Hosted pairing does not proxy traffic through Trifecta. The browser still connects directly to the backend URL in the pairing link.
