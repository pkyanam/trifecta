...
import {
  ...
  buildRemoteBelweaveRunnerScript,
...
describe("ssh tunnel scripts", () => {
  it("builds the remote Trifecta runner with npx and npm fallbacks", () => {
    const script = buildRemoteBelweaveRunnerScript({ nodeEngineRange: TEST_NODE_ENGINE_RANGE });

    assert.include(script, "BELWEAVE_NODE_SCRIPT_PATH=''");
    assert.include(script, 'exec trifecta "$@"');
    assert.include(script, "exec npx --yes '@belweave/trifecta@latest' \"$@\"");
    assert.include(script, "exec npm exec --yes '@belweave/trifecta@latest' -- \"$@\"");
    assert.include(script, "could not install '@belweave/trifecta@latest'");
    assert.include(script, 'prepend_path_if_dir "$HOME/.local/bin"');
    assert.include(script, `BELWEAVE_NODE_ENGINE_RANGE='${TEST_NODE_ENGINE_RANGE}'`);
    ...
    assert.include(script, 'for BELWEAVE_NODE_BIN in "$NVM_DIR"/versions/node/*/bin');
    assert.notInclude(script, "ensure $NVM_DIR/nvm.sh is available");
  });

  it("does not hard-code a remote node engine range", () => {
    const script = buildRemoteBelweaveRunnerScript();

    assert.include(script, "BELWEAVE_NODE_ENGINE_RANGE=''");
    assert.notInclude(script, TEST_NODE_ENGINE_RANGE);
  });

  it("shell-quotes package specs in the remote Trifecta runner", () => {
    const script = buildRemoteBelweaveRunnerScript({
  ...
  it("builds the remote Trifecta runner with a node script override", () => {
    ...
    assert.include(
      script,
      "BELWEAVE_NODE_SCRIPT_PATH='/Users/julius/Development/Work/codething-mvp/apps/server/dist/bin.mjs'",
    );
    assert.include(script, 'exec node "$BELWEAVE_NODE_SCRIPT_PATH" "$@"');
  });

  it("uses the remote Trifecta runner for launch and pairing scripts", () => {
    const target = {
      alias: "devbox",
    ...
    assert.include(
      buildRemoteLaunchScript({ nodeEngineRange: TEST_NODE_ENGINE_RANGE }),
      `BELWEAVE_NODE_ENGINE_RANGE='${TEST_NODE_ENGINE_RANGE}'`,
    );
    assert.include(
      buildRemoteLaunchScript({ nodeEngineRange: TEST_NODE_ENGINE_RANGE }),
