...
describe("ssh auth", () => {
  ...
  it.effect("creates askpass env for cached password prompts", () =>
    Effect.gen(function* () {
      ...
      const directory = yield* fs.makeTempDirectoryScoped({ prefix: "belweave-ssh-askpass-test-" });
      ...
      assert.equal(env.BELWEAVE_SSH_AUTH_SECRET, "super-secret");
      assert.equal(env.DISPLAY, "belweave");
  ...
  it.effect("builds a windows askpass launcher pair", () =>
    Effect.gen(function* () {
      const descriptor = yield* buildSshAskpassHelperDescriptor({
        directory: "C:\\temp\\belweave-ssh-askpass",
        platform: "win32",
      }).pipe(Effect.provide(NodeServices.layer));

      assert.equal(descriptor.launcherPath, "C:\\temp\\belweave-ssh-askpass\\ssh-askpass.cmd");
      assert.deepEqual(
