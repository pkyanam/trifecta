...
import type { DesktopSshEnvironmentTarget, DesktopUpdateChannel } from "@belweave/contracts";
...
const PUBLISHABLE_BELWEAVE_VERSION_PATTERN = /^\d+\.\d+\.\d+(?:-[0-9A-Za-z.-]+)?$/u;
...
export function resolveRemoteBelweaveCliPackageSpec(input: {
  readonly appVersion: string;
  readonly updateChannel: DesktopUpdateChannel;
  readonly isDevelopment?: boolean;
}): string {
  const appVersion = input.appVersion.trim();
  if (!input.isDevelopment && PUBLISHABLE_BELWEAVE_VERSION_PATTERN.test(appVersion)) {
    return `@belweave/trifecta@${appVersion}`;
  }
